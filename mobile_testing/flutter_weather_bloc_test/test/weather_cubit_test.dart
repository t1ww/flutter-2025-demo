  ...
  group('WeatherCubit', () {
    ...
    blocTest<WeatherCubit, WeatherState>(
      ...
      build: () {
        ...
        ).thenAnswer((_) async => Weather(city: 'Bangkok', temperature: 30, description: 'Cloudy'));
        return cubit;
      },
      act: (cubit) => cubit.fetchWeather('Bangkok'),
      expect: () => [WeatherLoading(), WeatherLoaded(city: 'Bangkok', temperature: 30, description: 'Cloudy')],
    );

    blocTest<WeatherCubit, WeatherState>(
      'emits [WeatherLoading, WeatherError] when fetchWeather throws',
      build: () {
        when(mockRepository.fetchWeatherServer('Bangkok')).thenThrow(Exception('error'));
        return cubit;
      },
      act: (cubit) => cubit.fetchWeather('Bangkok'),
